...
#define TITLE  "Boyer-Moore-Horspool algorithm\n"
#define AUTHOR "Jan Holub, 2023\n"
#define HRULE  "-------------------------------\n"
#define USAGE  "Usage: bmh <TEXT> <PATTERN>\n"
...
/* --------------------------------------------------------------------------*/
void boyer_moore_horspool(char* text, int n, char* pattern, int m) {
  int i, j, *BCS;
...
int main(int argc, char **argv) {
  ...
  n = strlen(argv[1]);
  m = strlen(argv[2]);
  boyer_moore_horspool(argv[1], n, argv[2], m);
  //  Boyer_Moore_Horspool(argv[1], n, argv[2], m);
